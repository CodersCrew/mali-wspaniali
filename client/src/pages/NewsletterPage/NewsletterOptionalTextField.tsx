...
export const NewsletterOptionalTextField: React.FC<{
    ...
    const handleDelete = (value: string) => {
        const filteredRecipients = recipients.value.filter(
    ...
    const setLabel = () => {
        switch (true) {
            case generalType.value === GeneralRecipientInputValues.parents &&
                specificType.value ===
                    SpecificRecipientInputValues.kindergarten:
                if (recipients.value.length > 0) {
                    return t(
                        'newsletter.recipient-select-kindergarten-label-filled',
                    );
                }
            ...
            default:
                if (recipients.value.length > 0) {
                    return t(
                        'newsletter.recipient-single-kindergarten-label-filled',
                    );
                }
    ...
    return (
        <TextField
            ...
            helperText={
                recipients.error ? t('newsletter.recipient-helper-text') : null
            }
            ...
            specificType.value === SpecificRecipientInputValues.kindergarten
                ? setMenuItems(
                      kindergartens.map(
                          (kindergarten) =>
                              `${kindergarten.name}, ${t(
                                  'newsletter.kindergarten-number',
                              )} ${kindergarten.number}`,
                      ),
                  )
